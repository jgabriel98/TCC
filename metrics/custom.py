...
def custom_loss(y_true, y_pred):
    ...
    return K.mean(se * cos_sim)  # * ((1.-movement_score)**2)
...
def custom_loss2(y_true, y_pred):
    ...
    return error * (tf.reduce_mean(c)**2) # ou entao: custom_mse(y_true, y_pred) * tf.reduce_mean(c)

def custom_specific_for_raw_price_prediction(y_true, y_pred):
    error = log_cosh(y_true[:,1:], y_pred)
